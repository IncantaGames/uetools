...
const commands: Command[] = [
  ...
  {
    command: "detectUnrealEngineInstallation",
    callback: detectUnrealEngineInstallation,
  },
  ...
  {
    command: "buildAndGenerateCompileCommands",
    callback: buildAndGenerateCompileCommands,
  },
  { command: "buildModule", callback: buildModule },
  { command: "showPluginInExplorer", callback: showPluginInExplorer },
  {
    command: "generateProjectFilesAndCompileCommands",
    callback: generateProjectFilesAndCompileCommands,
  },
  {
    command: "checkProjectAndUnrealInstallation",
    callback: checkProjectAndUnrealInstallation,
  },
  { command: "changeEngineVersion", callback: changeEngineVersion },
  {
    command: "changeEngineVersionRoutine",
    callback: changeEngineVersionRoutine,
  },
];
...
export function activate(context: vscode.ExtensionContext) {
  ...
  commands.forEach((c) => {
    context.subscriptions.push(
      vscode.commands.registerCommand(`uetools.${c.command}`, c.callback)
    );
  });

  // Register status bar items
  const projectName = vscode.window.createStatusBarItem(
