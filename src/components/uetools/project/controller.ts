...
export class ProjectViewController {
  ...
  private createOrShowPanel() {
    if (this._webviewView) {
      this._webviewView.webview.options = {
        ...
        localResourceRoots: [
          vscode.Uri.file(
            path.join(this._extensionContext.extensionPath, "dist")
          ),
          vscode.Uri.file(
            path.join(this._extensionContext.extensionPath, "res")
          ),
  ...
  private getHtml(): string {
    return `
        ...
        <head>
            <meta charset="UTF-8">
            <base href="${vscode.Uri.file(".").with({
              scheme: "vscode-resource",
            })}/">
            <script>
                window.acquireVsCodeApi = acquireVsCodeApi;
                window.WORKSPACE_URI = "${this._webviewView!.webview.asWebviewUri(
                  vscode.Uri.file(
                    vscode.workspace
                      .workspaceFolders![0].uri.fsPath.split(path.sep)
                      .join(path.posix.sep)
                  )
                )}";
                window.EXTENSION_URI = "${this._webviewView!.webview.asWebviewUri(
                  vscode.Uri.file(
        ...
        <body>
            <div id="root"></div>
            <script src="${this._webviewView!.webview.asWebviewUri(
              vscode.Uri.file(
                path.join(
                  this._extensionContext.extensionPath,
                  "dist",
