  ...
  return new Promise<boolean>((resolve, reject) => {
    (async () => {
      ...
      Context.set(
        ...
        path.join(unrealEngineInstallationSearchPath, engineFolder)
      );

      // set UnrealBuildTool, UnrealEditor and Mono path based on Unreal version.
      // get engine version as number
      const engineVersion = parseInt(
        project.EngineAssociation.replace("UE_", "")
      );
      if (engineVersion === 4) {
        if (os === "win32") {
          Context.set(
            "unrealBuildToolPath",
            path.join(
              Context.get("unrealEngineInstallation") as string,
          ...
          Context.set(
            ...
            path.join(
              Context.get("unrealEngineInstallation") as string,
              "Engine\\Binaries\\ThirdParty\\DotNet\\Windows\\dotnet.exe"
            )
          );
        } else if (os === "darwin") {
          Context.set(
          ...
          Context.set(
            ...
            path.join(
              Context.get("unrealEngineInstallation") as string,
              "Engine/Binaries/Mac/UE4Editor.app/Contents/MacOS/UnrealEditor"
            )
          );
          Context.set(
            "runtimePath",
        ...
        if (os === "win32") {
          ...
          Context.set(
            ...
            path.join(
              Context.get("unrealEngineInstallation") as string,
              "Engine/Binaries/ThirdParty/DotNet/Mac/dotnet"
            )
          );
        } else if (os === "linux") {
          Context.set(
        ...
        reject(
          new Error(`Unreal Engine ${project.EngineAssociation} not supported`)
        );
        return false;
      }
