 ...
 */
export async function generateCompileCommands(): Promise<void> {
  ...
  vscode.window.showInformationMessage(
    `Generating compile commands for ${project.Modules[0].Name}`
  );
  ...
  if (fs.existsSync(compileCommandsFile)) {
    await fs.promises.unlink(compileCommandsFile);
  }
  ...
  await new Promise<void>((resolve) => {
    vscode.tasks.onDidEndTask(async (e) => {
      if (e.execution.task === execution.task) {
        ...
        const newCompileCommandsFile = path.join(
          unrealEngineInstallation,
        ...
        await fs.promises.rename(newCompileCommandsFile, compileCommandsFile);

        // set compile_commands.json relative dir in clangd arguments in workspace settings
        const clangd = vscode.workspace
          .getConfiguration("clangd", vscode.workspace.workspaceFolders![0])
          .get("arguments") as string[];
        const clangdIndex = clangd.findIndex((arg) =>
