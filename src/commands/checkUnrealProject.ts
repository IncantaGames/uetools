          ...
          for (const file of files) {
            if (file.endsWith(".uproject")) {
              ...
              const project = JSON.parse(
                fs.readFileSync(`${folder.uri.fsPath}/${file}`, "utf8")
              ...
              if (fs.existsSync(`${folder.uri.fsPath}/Plugins`)) {
                const pluginDirs = fs.readdirSync(
                  `${folder.uri.fsPath}/Plugins`
                );
                project.ProjectPlugins = [];
                for (const pluginDir of pluginDirs) {
                  if (
                    fs.existsSync(
                      `${folder.uri.fsPath}/Plugins/${pluginDir}/${pluginDir}.uplugin`
                    )
                  ) {
