...
export async function buildModule(args: { moduleName: string }): Promise<void> {
  ...
  if (os === "win32") {
    ...
    shellCommand = new vscode.ShellExecution(
      `"${unrealBuildToolPath}" -mode=Build -ModuleWithSuffix=${
        args.moduleName
      },${randomIntNum} -ForceHotReload -project="${path.join(
        projectFolder,
    ...
    shellCommand = new vscode.ShellExecution(
      `${runtimePath.split(" ").join("\\ ")} ${unrealBuildToolPath
        .split(" ")
        .join("\\ ")} -mode=Build -ModuleWithSuffix=${
        args.moduleName
      },${randomIntNum} -ForceHotReload -project=${path
    ...
    vscode.tasks.onDidEndTask((e) => {
      if (e.execution.task === execution.task) {
        vscode.window.showInformationMessage(
          `Module ${args.moduleName} build completed`
        );
        resolve();
      }
    });
